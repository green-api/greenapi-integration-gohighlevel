...
3. **Настройте переменные окружения:**
   ...
   Необязательные переменные:
    ...
    - `OUTGOING_API_ECHO_DELAY_MS` (по умолчанию `3000`): задержка перед обработкой уведомления
      `outgoingAPIMessageReceived`. Она даёт адаптеру время зарегистрировать собственные отправки,
      чтобы они не попали в GHL дважды.
...
### Сообщения, отправленные вне GHL (с телефона или из других интеграций → GHL)

1. Сообщения, которые уходят с вашего номера WhatsApp без участия GHL, тоже добавляются в диалог GHL
   как исходящие, поэтому история переписки в GHL остаётся полной:
    - сообщения, отправленные из приложения WhatsApp на телефоне (`outgoingMessageReceived`)
    - сообщения, отправленные через [GREEN-API](https://green-api.com) любым другим приложением
      (`outgoingAPIMessageReceived`)
2. Поддерживаются все типы сообщений, перечисленные для входящих (текст, медиа, локация, контакты,
   опросы, кнопки, редактирование, удаление, реакции и т.д.).
3. Статусы доставки WhatsApp (`outgoingMessageStatus`) переносятся на соответствующее сообщение в
   GHL, поэтому в интерфейсе видно `delivered`, `read` и ошибки (`failed`, `noAccount`,
   `notInGroup`, `yellowCard`).
4. Сообщения, отправленные самим адаптером (из интерфейса GHL или через действие в workflow),
   определяются по ID сообщения GREEN-API и не дублируются.
5. Для этого на инстансе должны быть включены уведомления `outgoingMessageWebhook`,
   `outgoingAPIMessageWebhook` и `outgoingWebhook`. Новые инстансы получают их сразу, а уже
   существующие обновляются автоматически при запуске адаптера.
   **Примечание:** включение уведомлений перезапускает инстанс GREEN-API, настройки применяются в
